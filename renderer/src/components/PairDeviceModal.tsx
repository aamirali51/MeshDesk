...
export function PairDeviceModal({ isOpen, onClose, defaultTab = 'myCode' }: PairDeviceModalProps) {
  const { getPairingCode, pairWithCode, claimFileWithCode } = useData()
  ...
  useEffect(() => {
    ...
    const handleConnChanged = (data: unknown) => {
      const payload = data as { connected?: boolean } | null
      if (payload && payload.connected && statusRef.current === 'connecting') {
        finishSuccess(null)
      }
    }
    ...
    const unsub4 = ipc.on(EVENTS.CONNECTION_CHANGED, handleConnChanged)
      ...
      {activeTab === 'myCode' && (
        <div className='flex flex-col items-center space-y-4 py-2'>
          ...
          <p className='max-w-xs text-center text-[11px] text-muted-foreground'>
            Share this code or scan the QR Code to securely pair another device. The code expires
            after 15 minutes.
          </p>
        </div>
      )}
        ...
        <form onSubmit={handlePairSubmit} className='space-y-4 py-2'>
          <div className='space-y-2'>
            ...
            >
              ...
              Pairing Code or One-Time File Code
            </label>
            <Input
              id='pair-code-input'
              placeholder='e.g. MD-ABCD-EFGH-JKLM-NPQR or DROP-4A82-9X1B'
              value={inputCode}
      ...
      {activeTab === 'scan' && (
        <div className='flex flex-col items-center space-y-3 py-2'>
          ...
          <p className='text-center text-[11px] text-muted-foreground'>
            Point your camera at a MeshDesk QR Code to pair or claim files automatically.
          </p>
          {renderStatusPanel()}
        </div>
      )}
