...
const STATUS_STYLE: Record<TransferStatus, string> = {
  ...
  pending_approval: 'text-accent border-accent/30 bg-accent/10'
}
      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-xs text-muted-foreground'>
            Encrypted file transfers with SHA-256 integrity verification.
          </p>
        </div>
        ...
        <CardContent className='p-0 divide-y divide-border/40'>
          {transfers.length === 0 ? (
            <div className='p-12 text-center space-y-3'>
              ...
              <div className='space-y-1'>
                <h3 className='text-sm font-bold text-foreground'>No Transfers Yet</h3>
                <p className='text-xs text-muted-foreground max-w-sm mx-auto'>
                  Pick an online device and send a file, or accept an incoming request. Every byte
                  is verified before it is written to disk.
                </p>
              </div>
            </div>
          ) : (
                    ...
                    <div className='flex items-center justify-between gap-2'>
                      <div className='min-w-0'>
                        ...
                        <p className='text-[11px] text-muted-foreground'>
                          {t.direction === 'send' ? 'to' : 'from'} {t.peerName || 'Remote Peer'} ·{' '}
                          {formatBytes(t.fileSize)} · {formatTime(t.createdAt)}
                        </p>
                      </div>
                      <span
                      ...
                      <div className='space-y-1'>
                        ...
                        <div className='flex items-center justify-between text-[10px] font-mono text-muted-foreground'>
                          ...
                          <span className='flex items-center gap-2'>
                            {isActive && t.speed > 0 ? formatSpeed(t.speed) : null}
                    ...
                    {done && t.summary && (
                      <div className='flex items-center gap-1.5 text-[10px] font-mono text-status-online'>
                        ...
                        <span>
                          ...
                          {formatBytes(t.summary.bytesVerified ?? 0)} · checksum{' '}
                          {t.summary.checksum ? t.summary.checksum.slice(0, 12) + '…' : '—'}
                        </span>
                      </div>
                    )}
        ...
        <ContextMenu
          ...
          items={[
            ...
            },
            {
              ...
              disabled: !['active', 'queued', 'paused', 'interrupted', 'pending_approval'].includes(
                rowMenu.transfer.status
              )
            },
            {
