...
async function doCall(method: MethodName, params?: unknown): Promise<unknown> {
  ...
  if (!isBridgeAvailable) {
    ...
    return Promise.reject(new Error('MeshDesk P2P engine is only available in the desktop app'))
  }
...
function startBridge(): void {
  if (cleanup || !isBridgeAvailable) return
  ...
  window.bridge.onWorkerIPC(WORKER_SPECIFIER, (raw: unknown) => {
    ...
    if (msg.type === 'response') {
      ...
      if (!req) {
        console.log(`[IPC ${ts}] ?? ${msg.id} UNEXPECTED RESPONSE`, msg.error || msg.result)
        return
      }
      ...
      if (msg.error) {
        ...
        console.log(`[IPC ${ts}] << ${msg.id}`, msg.result !== undefined ? msg.result : '')
